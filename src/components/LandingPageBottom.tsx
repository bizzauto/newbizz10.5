...
const Section: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className = '' }) => {
  ...
  return (
    <div ref={ref} className={`transition-all duration-700 ${inView ? 'opacity-100 translate-y-0' : ''} ${className}`}>
      {children}
    </div>
  );
};
...
const LandingPageBottom: React.FC<Props> = ({ openFaq, setOpenFaq, showBackToTop }) => (
  <>
    ...
    {/* Back to Top */}
    {showBackToTop && (
      <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="fixed bottom-20 right-6 z-40 p-3 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl shadow-lg shadow-emerald-500/25 hover:-translate-y-1 transition-all animate-fade-in-up cursor-pointer">
